...
export const restartByPm2ID = async (pm2ID) => {
    try {
        await execPromise(`pm2 restart ${pm2ID}`);
        console.log(`برنامه با ID ${pm2ID} با موفقیت ریستارت شد.`);
    } catch (error) {
        console.error(`خطا در ریستارت برنامه با ID ${pm2ID}: ${error.message}`);
    }
}
